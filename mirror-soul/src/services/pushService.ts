...
export const registerPushDevice = async (
  ...
  // installationId는 기기에 지속적으로 남는 식별자라 로그에 남기지 않는다(개발 콘솔에서도).
  logger.debug('registerPushDevice:', { platform: data.platform });
  try {
    const response = await apiClient.put<RegisterPushDeviceResponse>('/push/devices', data);
    logger.info('registerPushDevice SUCCESS:', { isSuccess: response.data.isSuccess });
...
export const unregisterPushDevice = async (installationId: string): Promise<UnregisterPushDeviceResponse> => {
  logger.debug('unregisterPushDevice');
  try {
