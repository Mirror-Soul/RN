...
7. **`app.json`의 `bundleIdentifier`/`package`가 한동안 `com.mirrorsoul.app`(확정 전 placeholder)였다 — 지금은 `com.mirrorsoul64.app`로 확정됨(2026-08-19, Firebase Console에 등록된 값과 통일).** `android/`·`ios/`는 둘 다 gitignored라 `app.json`을 고쳐도 이미 생성된 네이티브 프로젝트엔 반영 안 된다 — 과거에 이 값으로 실기기 서명(프로비저닝 프로파일)을 시도한 흔적이 있어서, 오래된 로컬 빌드 산출물에서 이 문자열을 다시 보게 되면 그냥 stale한 것이니 당황하지 말 것(항목 8 참고).
8. **`app.json`의 네이티브 관련 설정(plugins, bundleIdentifier/package, permissions 등)을 바꿔도 `npx expo start`나 `npx expo run:ios/android -d`가 자동으로 기존 `android/`·`ios/`에 동기화해주지 않는다 — 두 플랫폼 다 `npx expo prebuild --clean`을 먼저 실행해야 한다.** Android만 해당된다고 착각해서 iOS 쪽엔 이 단계를 안내하지 않았다가, 사용자가 몇 달 전 placeholder 패키지명으로 서명 시도 → 실패를 한 번 겪었다(항목 7의 stale 값이 원인). `app.json`을 native 관련해서 고쳤으면 두 플랫폼 다 `prebuild --clean`부터 하고 시작할 것.
9. **(2026-08-19) 무료(개인) Apple 개발자 계정은 Push Notifications capability(`aps-environment` entitlement)를 지원하지 않아, `expo-notifications` 플러그인을 `app.json`에 추가하면 실기기 서명이 `"Personal development teams... do not support the Push Notifications capability"`로 항상 실패한다.** `expo-notifications`의 iOS 플러그인(`withNotificationsIOS.ts`)은 이 entitlement를 무조건 추가하고 끄는 옵션이 없다. `mirror-soul/plugins/withDisableIOSPushEntitlement.js`(기존 `withSentryDisableAutoUpload.js`와 같은 커스텀 플러그인 패턴)로 이 entitlement를 iOS에서만 제거해 해결함 — 유료 Apple Developer Program으로 iOS 푸시를 시작하면 이 플러그인을 `app.json`의 `plugins` 배열에서 제거할 것.
