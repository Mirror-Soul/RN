...
1. **`npx expo prebuild --clean`이 크래시** — `app.config.js`의 `expo-font` 플러그인은 `{fontFamily, fontDefinitions}` 객체가 아니라 **경로 문자열 배열**을 받아야 한다. 이미 고쳐져 있음 — 같은 실수를 반복하지 말 것.
2. **빌드 마지막 단계(`Bundle React Native code and images`)에서 `sentry-cli` 실패** — 실제 Sentry 프로젝트가 없으면 소스맵 자동 업로드가 항상 실패한다. `mirror-soul/plugins/withSentryDisableAutoUpload.js`가 prebuild 시 `ios/.xcode.env.local`에 `SENTRY_DISABLE_AUTO_UPLOAD=true`를 자동으로 써준다 (이미 `app.config.js` plugins에 등록되어 있음). 실제 Sentry 프로젝트를 연동하면 이 플러그인을 제거할 것.
...
7. **`app.config.js`의 `bundleIdentifier`/`package`가 한동안 `com.mirrorsoul.app`(확정 전 placeholder)였다 — 지금은 `com.mirrorsoul64.app`로 확정됨(2026-08-19, Firebase Console에 등록된 값과 통일).** `android/`·`ios/`는 둘 다 gitignored라 `app.config.js`을 고쳐도 이미 생성된 네이티브 프로젝트엔 반영 안 된다 — 과거에 이 값으로 실기기 서명(프로비저닝 프로파일)을 시도한 흔적이 있어서, 오래된 로컬 빌드 산출물에서 이 문자열을 다시 보게 되면 그냥 stale한 것이니 당황하지 말 것(항목 8 참고).
8. **`app.config.js`의 네이티브 관련 설정(plugins, bundleIdentifier/package, permissions 등)을 바꿔도 `npx expo start`나 `npx expo run:ios/android -d`가 자동으로 기존 `android/`·`ios/`에 동기화해주지 않는다 — 두 플랫폼 다 `npx expo prebuild --clean`을 먼저 실행해야 한다.** Android만 해당된다고 착각해서 iOS 쪽엔 이 단계를 안내하지 않았다가, 사용자가 몇 달 전 placeholder 패키지명으로 서명 시도 → 실패를 한 번 겪었다(항목 7의 stale 값이 원인). `app.config.js`을 native 관련해서 고쳤으면 두 플랫폼 다 `prebuild --clean`부터 하고 시작할 것.
9. **(2026-08-19) 무료(개인) Apple 개발자 계정은 Push Notifications capability(`aps-environment` entitlement)를 지원하지 않아, `expo-notifications` 플러그인을 `app.config.js`에 추가하면 실기기 서명이 `"Personal development teams... do not support the Push Notifications capability"`로 항상 실패한다.** `expo-notifications`의 iOS 플러그인(`withNotificationsIOS.ts`)은 이 entitlement를 무조건 추가하고 끄는 옵션이 없다. `mirror-soul/plugins/withDisableIOSPushEntitlement.js`(기존 `withSentryDisableAutoUpload.js`와 같은 커스텀 플러그인 패턴)로 이 entitlement를 iOS에서만 제거해 해결함 — 유료 Apple Developer Program으로 iOS 푸시를 시작하면 이 플러그인을 `app.config.js`의 `plugins` 배열에서 제거할 것.
   - **여기서 처음엔 순서를 반대로 둬서 한 번 실패했다(직접 재현하고 소스 추적으로 근본 원인 확정함)**: `@expo/config-plugins`의 mod 체이닝(`withMod.js`)은 `plugins` 배열에서 **나중에 오는 플러그인의 action이 먼저 실행되고, 그 결과를 `nextMod`로 앞쪽(먼저 등록된) 플러그인에 넘기는 구조**다(양파처럼 안쪽→바깥쪽 실행, **먼저 등록된 플러그인이 최종 결과를 결정**). `withDisableIOSPushEntitlement`를 `expo-notifications`보다 **뒤에** 뒀더니, 내 삭제 action이 먼저(entitlements가 아직 비어있을 때) 실행되어 아무 효과가 없었고 그다음 `expo-notifications`의 추가 action이 실행되어 키가 그대로 남았다. `expo-notifications`보다 **앞으로** 옮겨야 내 삭제가 나중에 실행되어 최종적으로 이긴다 — `prebuild` 후 생성된 `ios/mirrorsoul/mirrorsoul.entitlements`가 빈 `<dict/>`인 것으로 직접 검증함. **이런 종류의 "플러그인 A가 플러그인 B의 네이티브 설정을 되돌리는" 커스텀 플러그인을 또 만들 일이 있으면, 배열 순서가 직관과 반대라는 점을 반드시 기억할 것.**
10. **(2026-09-22) Claude Code의 Bash 도구 환경엔 `LANG`/`LC_ALL`이 아예 비어 있어(`locale`이 전부 `"C"`), `pod install`이 `Unicode Normalization not appropriate for ASCII-8BIT`로 즉시 크래시한다.** `npx expo prebuild`/`npx expo run:ios`가 내부적으로 호출하는 `pod install`도 동일하게 실패함 — CocoaPods 자신도 이 상황에서 "터미널이 UTF-8이 아니다"라는 경고를 띄운다. 해결: 관련 명령 앞에 `LANG=en_US.UTF-8 LC_ALL=en_US.UTF-8`를 매번 붙일 것(`export`는 Bash 도구 호출 사이에 유지되지 않으므로 한 번 export해두는 방식은 안 통함 — 명령마다 인라인으로 붙여야 함). `npx expo run:ios`/Xcode GUI를 사용자가 직접 터미널에서 실행할 때는 보통 로그인 셸의 로케일을 상속하므로 원래 문제없었을 가능성이 높다.
11. **(2026-09-22) 이 Mac의 Xcode 26.6에서 `npx expo run:ios -d`/`--device <UDID나 이름>`이 항상 `CommandError: No device UDID or name matching "..."`로 실패한다** — 직전에 `Unexpected devicectl JSON version output from devicectl. Connecting to physical Apple devices may not work as expected.` 경고가 뜨는데, Expo CLI가 이 Xcode 버전의 `devicectl` JSON 출력 스키마를 못 읽어서 디바이스 목록 자체를 못 얻는 게 근본 원인으로 보인다(UDID로 지정해도, `xcrun devicectl list devices`가 "available (paired)"로 보여주는 이름으로 지정해도 둘 다 실패 — Expo CLI 쪽 파싱 문제이지 디바이스 연결 문제가 아님). **우회법**: `expo run:ios`를 아예 쓰지 말고 `xcodebuild`를 직접 호출한다 — 이러면 디바이스를 지정할 필요가 없다(빌드만 하고 설치는 항목 #3의 `ios-deploy`가 따로 담당하므로):
    ```bash
    cd mirror-soul/ios
    LANG=en_US.UTF-8 LC_ALL=en_US.UTF-8 xcodebuild -workspace mirrorsoul.xcworkspace -scheme mirrorsoul \
      -configuration Debug -destination 'generic/platform=iOS' -allowProvisioningUpdates \
      DEVELOPMENT_TEAM=<TEAM_ID> build
    ```
    `DEVELOPMENT_TEAM`이 없으면 `Signing for "mirrorsoul" requires a development team`로 실패한다(자동 서명이라도 팀 ID는 명시가 필요) — `security find-identity -v -p codesigning`으로 이 Mac에 등록된 "Apple Development" 인증서는 확인되지만 그게 보여주는 계정과 실제 앱에 서명해온 팀 ID가 다를 수 있다(이 프로젝트는 `com.mirrorsoul64.app`용 팀 ID가 `TR75X684HM` — Apple Developer 계정 `tlsdnr303@gmail.com`의 개인 무료 팀). 확인하려면 `~/Library/Developer/Xcode/UserData/Provisioning Profiles/*.mobileprovision`가 남아있는 경우 `security cms -D -i <경로> | plutil -extract TeamIdentifier xml1 -o - -`로 뽑을 수 있다. `-allowProvisioningUpdates`가 있으면 프로파일이 없거나 만료됐어도 빌드 중에 자동 갱신한다(무료 팀 프로파일은 보통 7일 정도로 금방 만료되니 당황하지 말 것 — 다음 빌드 때 알아서 새로 받아온다).
